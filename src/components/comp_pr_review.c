...
static void draw(xf_draw_ctx_t *ctx, int w, int h, void *user_data)
{
    ...
    for (int i = 0; i < d->count; i++) {
        ...
        xf_draw_text(ctx, r->initials, 14.0, cy + 4.5, &(xf_text_opts_t){
            .size = FONT_XS, .weight = 700, .color = t->white, .align = XF_TEXT_CENTER
        });
        ...
        xf_draw_text(ctx, r->age, (double)w - 8.0, cy + 5.0, &(xf_text_opts_t){
        ...
        if (r->reviews > 0) {
            ...
            double bw = xf_draw_measure_text(ctx, badge, FONT_SM, 600) + 8.0;
            double bx = (double)w - 8.0 - bw
                        - xf_draw_measure_text(ctx, r->age, FONT_MD, 400) - 6.0;
            xf_draw_fill_round_rect(ctx, bx, cy - 7.0, bw, 14.0, 6.0, t->success_bg);
            xf_draw_text(ctx, badge, bx + bw / 2.0, cy + 5.0, &(xf_text_opts_t){
                .size = FONT_SM, .weight = 600, .color = t->success_fg, .align = XF_TEXT_CENTER
            });
        }
