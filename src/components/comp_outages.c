/**
 * @file comp_outages.c
 * @brief Section header + list of active outage rows with severity colours.
 *
 * Each row carries its own colour set so this function applies them
 * uniformly without encoding any severity-to-colour mapping here.
 */
...
static void draw(xf_draw_ctx_t *ctx, int w, int h, void *user_data)
{
    ...
    for (int i = 0; i < d->count; i++) {
        ...
        /* Duration to the left of the pill */
