...
static void blit(xf_device_t *dev, const uint8_t *frame, int fw,
                 int x, int y, int w, int h, uint8_t *scratch)
{
    if (!dev)
        return;
                            ...
                            const uint8_t *new_f, int w, int h, uint8_t *scratch)
{
    ...
    for (int s = 0; s < WIPE_STEPS; s++) {
        ...
        blit(dev, new_f, w, 0, y, w, bh, scratch);
    }
}
                             ...
                             const uint8_t *new_f, int w, int h, uint8_t *scratch)
{
    ...
    for (int s = 0; s < WIPE_STEPS; s++) {
        ...
        blit(dev, new_f, w, x, 0, bw, h, scratch);
    }
}
 ...
 */
static int make_checker(const uint8_t *src_a, const uint8_t *src_b,
                     ...
                     xf_transition_t effect)
{
    if (effect < 0 || effect >= XF_TRANS_COUNT)
        return;
    uint8_t *scratch = malloc((size_t)(width * height * 3));
    if (!scratch)
        return;
    effects[effect](dev, old_frame, new_frame, width, height, scratch);
    free(scratch);
}
