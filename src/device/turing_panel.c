...
{
    if (turing_proto_send_hello(dev->fd) < 0) return -1;

    uint8_t resp[TURING_RESP_LEN];
    int n = turing_proto_read(dev->fd, resp, TURING_RESP_LEN);
    ...
    if (n == TURING_RESP_LEN &&
        ...
        dev->display_width  = 320;
        dev->display_height = 480;
...
xf_device_t *panel_open_auto(void)
{
    ...
    if (port_detect_auto(port, sizeof(port)) < 0) {
        fprintf(stderr, "panel_open_auto: no Turing device found\n");
        return NULL;
    }
...
int panel_set_orientation(xf_device_t *dev, xf_orientation_t orientation)
{
    ...
    int w = effective_width(dev);
    int h = effective_height(dev);
    return turing_proto_send_orient(dev->fd, (int)orientation, w, h);
}
...
int panel_set_brightness(xf_device_t *dev, int level)
{
    ...
    return turing_proto_send_cmd(dev->fd, TURING_CMD_SET_BRIGHTNESS, val, 0, 0, 0);
}
...
{
    return turing_proto_send_cmd(dev->fd, TURING_CMD_SCREEN_OFF, 0, 0, 0, 0);
}

int panel_screen_on(xf_device_t *dev)
{
    return turing_proto_send_cmd(dev->fd, TURING_CMD_SCREEN_ON, 0, 0, 0, 0);
}
                         ...
                         const uint8_t *rgb888)
{
    ...
    if (dev->orientation == XF_ORIENT_PORTRAIT ||
        ...
        if (!rotated) return -1;
        turing_image_rotate_180(rgb888, width, height, rotated);
        pixels = rotated;
    }

    turing_proto_send_cmd(dev->fd, TURING_CMD_DISPLAY_BITMAP, x0, y0, x1, y1);
    ...
    turing_image_rgb888_to_rgb565le(pixels, pixel_count, rgb565);
    ...
    while (offset < total) {
        ...
        turing_proto_send_raw(dev->fd, rgb565 + offset, chunk);
        offset += chunk;
    }
