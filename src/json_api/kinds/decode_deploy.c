...
int decode_deploy(xf_str_slice_t s, void *vdata, int patch)
{
    ...
    if (xf_decode_str(s, "$.time_ago", XF_STR_BUF(d->time_ago, sizeof d->time_ago), patch) < 0) return -1;
    if (xf_decode_str(s, "$.label",    XF_STR_BUF(d->label, sizeof d->label), patch) < 0) return -1;
    int status = d->status;
