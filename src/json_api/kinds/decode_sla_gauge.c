...
static int decode_sla_row(xf_str_slice_t s, void *elem, int patch)
{
    ...
    if (xf_decode_str  (s, "$.label",   XF_STR_BUF(r->label, sizeof r->label), patch) < 0) return -1;
    if (xf_decode_str  (s, "$.value",   XF_STR_BUF(r->value, sizeof r->value), patch) < 0) return -1;
