...
int decode_build_status(xf_str_slice_t s, void *vdata, int patch)
{
    ...
    if (xf_decode_str  (s, "$.build_id",     d->build_id,     sizeof d->build_id,     patch) < 0) return -1;
    if (xf_decode_str  (s, "$.duration",     d->duration,     sizeof d->duration,     patch) < 0) return -1;
    if (xf_decode_str  (s, "$.status",       d->status,       sizeof d->status,       patch) < 0) return -1;
