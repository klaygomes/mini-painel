...
static int decode_outage_row(xf_str_slice_t s, void *elem, int patch)
{
    ...
    if (xf_decode_str  (s, "$.service",  r->service,  sizeof r->service,  patch) < 0) return -1;
    if (xf_decode_str  (s, "$.duration", r->duration, sizeof r->duration, patch) < 0) return -1;
    if (xf_decode_str  (s, "$.status",   r->status,   sizeof r->status,   patch) < 0) return -1;
