...
static int decode_alert_row(const char *s, int len, void *elem, int patch)
{
    comp_alert_row_t *r = elem;
    if (xf_decode_str  (s, len, "$.message", r->message, sizeof r->message, patch) < 0) return -1;
...
int decode_alerts(const char *s, int len, void *vdata, int patch)
{
