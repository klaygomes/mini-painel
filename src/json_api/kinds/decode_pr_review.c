...
static int decode_pr_row(xf_str_slice_t s, void *elem, int patch)
{
    ...
    if (xf_decode_str  (s, "$.initials",    r->initials,    sizeof r->initials,    patch) < 0) return -1;
    if (xf_decode_str  (s, "$.title",       r->title,       sizeof r->title,       patch) < 0) return -1;
    if (xf_decode_str  (s, "$.age",         r->age,         sizeof r->age,         patch) < 0) return -1;
